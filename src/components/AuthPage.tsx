        ...
        {view === "login" && (
          <>
            ...
            <PasswordInput placeholder="Password" value={password} onChange={e => setPassword(e.target.value)}
              className="mb-3" autoComplete="current-password" onKeyDown={e => e.key === "Enter" && handleLogin()} />
            <label className="flex items-center justify-between mb-4 text-left cursor-pointer select-none">
              ...
              {(getRememberedEmail() || getRememberFlag()) && (
                <button
                  type="button"
                  onClick={() => {
                    clearRememberedEmail();
                    setEmail("");
                    setRememberMe(false);
                    toast.success("Email forgotten on this device.");
                  }}
            ...
            <Button onClick={handleLogin} disabled={loading} className="w-full mb-3">
              {loading ? "Signing in..." : <EditableText textKey="auth.signin_btn" defaultValue="Sign In" />}
            </Button>
            <Button onClick={handleGoogle} variant="outline" className="w-full mb-3" disabled={isInAppBrowser()}>
              ...
              <EditableText textKey="auth.google_btn" defaultValue="Continue with Google" />
            </Button>
            <p className="text-sm text-muted-foreground">
              <EditableText textKey="auth.new_prompt" defaultValue="New?" /> <button onClick={() => { setView("signup"); setError(""); }} className="text-secondary font-semibold underline"><EditableText textKey="auth.signup_link" defaultValue="Sign Up" /></button>
              {" | "}
              <button onClick={() => { setView("forgot"); setError(""); }} className="text-secondary font-semibold underline"><EditableText textKey="auth.forgot_link" defaultValue="Forgot Password?" /></button>
            </p>
          </>
        )}
        ...
        {view === "signup" && (
          <>
            <h2 className="text-xl font-semibold text-primary mb-4">
              <EditableText textKey="auth.signup_title" defaultValue="Join Students for Success" />
            </h2>
            ...
            <Button onClick={handleSignup} disabled={loading} className="w-full mb-3">
              {loading ? "Creating..." : <EditableText textKey="auth.register_btn" defaultValue="Register" />}
            </Button>
            <Button onClick={handleGoogle} variant="outline" className="w-full mb-3" disabled={isInAppBrowser()}>
              <EditableText textKey="auth.google_btn" defaultValue="Continue with Google" />
            </Button>
